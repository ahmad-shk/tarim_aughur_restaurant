    ...
    <main>
      {(t.categories as MenuCategory[]).map((category) => (
        <section key={category.id} className="py-7 md:py-10 px-4" id={category.id}>
          <h2 className="aboreto-text dark:text-secondary text-center text-[35px] md:mb-[50px] mb-[25px]">{category.name}</h2>
          ...
          <div className="space-y-10">
            {category.sections.map((section, sectionIndex) => (
              ...
              >
                <div className="image rounded-md overflow-hidden cursor-pointer" onClick={() => setIsModalOpen(true)}>
                  <Image
